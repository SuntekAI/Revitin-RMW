...
async function fetchAndProcessShopifyOrders(
  ...
  const initialUrl = `${shopifyStoreUrl}/orders.json?created_at_min=${minTimestamp}&created_at_max=${
  ...
  while (nextUrl) {
    try {
      ...
      const maxOrderTime = Math.max(
        ...orders.map((order) => new Date(order.created_at).getTime())
      );
...
async function upsertOrder(order) {
  ...
  const lineItemsData = order.line_items.map((item) => ({
    ...
    pre_tax_price: parseFloat(item.pre_tax_price), // Ensure pre_tax_price is a float
    variant_title: item.variant_title || null,
    product_exists: item.product_exists,
    total_discount: parseFloat(item.total_discount), // Ensure total_discount is a float
...
export async function createNewOrders() {
  const lastUpdate = await prisma.last_order_update.findFirst({
